...
def prune_leaves(t, vals):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_leaf(t) and (label(t) in vals):   #base case
        return None             
    new_branches = []                       #creating empty list
    for b in branches(t):
        new_branch = prune_leaves(b, vals)  #b is the smaller tree from t
        if new_branch:                      # if new_branch is True, it will be added
...
def ordered_digits(x):
    ...
    def helper(n):
        if n<10:
            return True
        elif n%10 < n%100//10: # if most recent digit is less than the next one
            return False
        return helper(n//10)
...
def make_repeater(func, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return accumulate(composer, lambda x: x, n, lambda k: func)
