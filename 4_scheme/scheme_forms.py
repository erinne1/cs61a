...
def do_lambda_form(expressions, env):
    ...
    #reminder that class LambdaProcedure is initialized: def __init__(self, formals, body, env): 
...
'''
    ...
    while copy.rest is not nil:
        copy.first = scheme_eval(copy.first, env)         #update copy.first so that scheme_eval saves it.....

        if copy.first is False:
            return copy.first        
        copy=copy.rest
        
    #return scheme_eval(copy.first, env) 
    return eval_all(copy,env)
'''
...
def do_or_form(expressions, env):
    ...
    if expressions is nil or expressions is False:
        return False

    while expressions.rest is not nil:
        
        if scheme_eval(expressions.first, env)  is not False:  #bc this accounts for 0, which is a Truthy value in Scheme's world
            return scheme_eval(expressions.first, env) 
        expressions=expressions.rest
...
'''
    ...
    while copy.rest is not nil:
        x = scheme_eval(copy.first, env)         #update copy.first so that scheme_eval saves it.....
        
        if x is not False:  #bc this accounts for 0, which is a Truthy value in Scheme's world
            return x
        copy=copy.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        clauseRest = clause.rest

        names = Pair(clause.first, names)
        vals = Pair(scheme_eval(clauseRest.first, env), vals)
